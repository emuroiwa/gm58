...
<br>Instalments is <strong>$ $instalments</strong> Monthly
<br>
...
$before=clean($_POST['amount']);
$after=clean($_POST['amount1']);
...
if(ValidateDate($purchasedate)==false){
	...
	exit;
		 ...
		 $monthdate=GetInstalmentMonthDate($purchasedate,$monthspaid);
		  $r = mysql_query("SELECT * FROM payment where   d='Balance_Before_VAT' and stand='$stand' GROUP BY stand ")or die(mysql_query());
if(mysql_num_rows($r)==1){
			   mysql_query("update payment set cash='$before',month='$monthname',year='$monthspaid' where stand='$stand' AND value_date='$monthdate' and payment_type<>'Deposit' and d='Balance_Before_VAT'") or die (mysql_error());
			   //after
			   mysql_query("update payment set cash='$after',month='$monthname',year='$monthspaid',value_date='$monthdate' where stand='$stand' and payment_type<>'Deposit' and d='Balance_After_VAT'") or die (mysql_error());
			 //Write to log file
			 WriteToLog("Balance Take Up AMENDMENT Amount=$before |Stand = $standno",$_SESSION['username']);
			 WriteToLog("Balance Take Up AMENDMENT Amount=$after |Stand = $standno",$_SESSION['username']);
			  
	}
		 if(mysql_num_rows($r)==0){
			 //before
    mysql_query("INSERT INTO `payment` (`cash`, `stand`, `date`, `capturer`, `month`, `year`, `payment_type`, `d`, `payment_date`, `value_date`) VALUES ('$before', '$stand', NOW(), '$_SESSION[name]', '$monthname', '$monthspaid', 'Credit','Balance_Before_VAT', now(), '$monthdate')") or die (mysql_error());
	
	
	//after
	    mysql_query("INSERT INTO `payment` (`cash`, `stand`, `date`, `capturer`, `month`, `year`, `payment_type`, `d`, `payment_date`, `value_date`) VALUES ('$after', '$stand', NOW(), '$_SESSION[name]', '$monthname', '$monthspaid', 'Credit','Balance_After_VAT', now(),  '$monthdate')") or die (mysql_error());

	//Write to log file
			 WriteToLog("Balance Take Up Amount=$before |Stand = $standno",$_SESSION['username']);
			 WriteToLog("Balance Take Up Amount=$after |Stand = $standno",$_SESSION['username']);
			
		 }
		 	 
		   mysql_query("update stand set instalments='$instalment',vatdate='$purchasedate',vat='YES',datestatus='$_POST[date]' where id_stand='$stand'") or die (mysql_error());
...
<script src="ui.full_datepickerlong.js" type="text/javascript" charset="utf-8"></script>
...
<tr>
  <td width="109"> <span class="style1 style9">Balance Paid <strong>BEFORE</strong> VAT :</span></td>
  <td width="150">
   $ <input type="text" name="amount" id="amount"  required /></td>
</tr><tr>
  <td width="109"> <span class="style1 style9">Purchase Date</span></td>
  <td width="150">
   &nbsp;&nbsp; <input type="text" name="date" id="date"/></td>
</tr><tr>
  <td width="109"> <span class="style1 style9">New Instalment After VAT:</span></td>
  <td width="150">
   $ <input type="text" name="instalment" id="instalment"  required /></td>
</tr><tr>
  <td width="109"> <span class="style1 style9">VAT Effected On :</span></td>
  <td width="150">
    &nbsp;&nbsp;&nbsp;<input type="text" name="date1" id="date1" required/></td>
</tr><tr>
  <td width="109"> <span class="style1 style9">Balance Paid <strong>AFTER</strong> VAT :</span></td>
  <td width="150">
   $ <input type="text" name="amount1" id="amount1"  required /></td>
</tr>
