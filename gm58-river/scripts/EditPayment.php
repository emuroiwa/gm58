...
//INTEREST CHECKING*********************************************************************************************************************************************************
$dt = strtotime("$date - 3month");
$threemonthsago = date('Y-m-d H:i:s',$dt);
//echo $threemonthsago."-".lastmonthdate($stand)."--------".lastInterestPayment($stand);
//exit;
if(lastmonthdate($stand)<$threemonthsago and lastInterestPayment($stand)<$threemonthsago){
	 WriteToLog("Interest Required for stand $standno",$_SESSION['username']);
					   	msg('Stand is in 3 Months arrears, Last Payment Date was '.lastmonthdate($stand));
			 link1("index.php?page=interest.php&id=".$stand); 
	}
...
$instalment=clean($_POST['instalment']);
 //debit
  $debit=getcredit($stand) + $balance;
    $monthz=GetInstalmentMonth($purchasedate,GetMonthsPaid($debit,$instalments));
$NumberOfMonths=GetMonthsPaid($debit,$instalments);
 $monthzDate=GetInstalmentMonthDate($purchasedate,GetMonthsPaid($debit,$instalments));
    mysql_query("INSERT INTO `payment` (`cash`, `stand`, `date`, `capturer`, `month`, `year`, `payment_type`, `d`, `payment_date`, `value_date`, `description`) VALUES ('$balance', '$stand', NOW(), '$_SESSION[name]', '$monthz', '$NumberOfMonths', 'Credit','Balance_After_VAT', now(), '$monthzDate','$_POST[txtDescription]')") or die (mysql_error());
	//Write to log file
			 WriteToLog("Payment Amount=$balance |Stand = $standno",$_SESSION['username']);
