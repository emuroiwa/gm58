...
$xmlString.="</SMS>"; //echo $xmlString; exit;  
...
//inserting in sms response
$sms4 = mysql_query("SELECT * FROM clients , stand , owners WHERE clients.id = owners.client_id AND stand.id_stand = owners.stand_id and contact not in(select mobile from sms_response where month='$month') ")or die(mysql_query());	

  while($rowsms4 = mysql_fetch_array($sms4))
  {  $msgsms4="Good Day. $rowsms[name] $rowsms[surname] Please be advised that your Northgate Heights Instalment of $ $rowsms[instalments]  for Stand# $rowsms[number] is Due on $smsdate.Thank You";
