...
*
FROM cashoutowners,cashoutstand,cashoutclients
where id_stand='$stand' and cashoutowners.client_id = cashoutclients.id AND
cashoutowners.stand_id = cashoutstand.id_stand
ORDER BY
cashoutclients.id ASC
...
*
FROM cashoutowners,cashoutclients,cashoutstand
where cashoutowners.stand_id='$stand' and cashoutowners.client_id = cashoutclients.id AND
cashoutowners.stand_id = cashoutstand.id_stand
 ")or die(mysql_query());
...
FROM `cashoutpayment`
WHERE
stand='$_GET[id]' and payment_type='credit' order by id ASC  ")or die(mysql_query());
...
echo "<tr><td>{$PaymentDate}</td><td>{$monthz}</td><td>{$row['d']} {$row['value_date']}</td><td align='right'>".zva($s)."</td><td align='right'>".zva($creditbalance)."</td><td align='right'>".zva($balance2)."</td></tr>";
 ...
 <tr bgcolor="#CCCCCC">
   ...
   <strong> <td></td>
    <td></td>
    <td> <h4><font color="#FF0000" >CASH OUT AMOUNT $<?php echo zva($CashOutAmount);?></font></h4></td>
    <td></td>
    <td><strong>AMOUNT DUE</strong></td>
    <td><strong><?php echo zva($balance);?></strong></td>
    </strong>
  </tr>
