...
<body ><center><strong>PAGE <?php echo $_POST['stand'];?></strong><hr>
...
include ('../aut.php');
	 ...
	 // include ('process.php'); 
	  ?><h3 align="center">COMPREHENSIVE LEDGER PAYMENT IN PROGRESS AS AT3 <?php echo $new;?></h3><hr>
<?php
	   //debit
//$stand=$_GET['id'];
$overal=mysql_query("SELECT * FROM `stand` WHERE status = 'payment_in_progress'   order by number asc limit $_POST[stand] ");
while($rowo = mysql_fetch_array($overal))
  { 
$stand=$rowo['id_stand'];
//echo mysql_num_rows($overal);
//nae andd surname
...
FROM `payment`
WHERE
payment.stand='$stand' and payment_type='credit' ")or die(mysql_query());
