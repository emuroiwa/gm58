...
*/
		 ...
		 $sms3 = mysql_query("SELECT * FROM clients , stand , owners WHERE clients.id = owners.client_id AND stand.id_stand = owners.stand_id ")or die(mysql_query());	
...
while($rowsms = mysql_fetch_array($sms3))
  { 
  $msgsms="Good Day. $rowsms[name] $rowsms[surname] Please be advised that Northgate has a new Stanbic Gweru acc 0240000904601 code 3503. Swift address SBICZWHX";
...
$xmlString.="
	...
	echo ("<SCRIPT LANGUAGE='JavaScript'> window.alert('Sms sent')
		 location='index.php'
		 	</SCRIPT>"); 
