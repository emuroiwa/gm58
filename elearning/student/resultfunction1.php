...
while($row1 = mysql_fetch_array($rs1)){
 ...
 function subjectresult($subject_id,$subject,$year,$term){
  global $reg;
  $mark= mysql_query("SELECT * FROM results ,student ,student_class WHERE results.reg = student.reg AND results.term ='$term' AND results.session ='$year' AND results.reg = '$reg'  AND results.`open` = 'no' AND student.reg = student_class.student and subject_id='$subject_id' and subject='$subject'  ")or die(mysql_query());
...
function subjecteffort($subject_id,$subject,$year,$term){
  global $reg;
  $mark = mysql_query("SELECT * FROM results ,student ,student_class WHERE results.reg = student.reg AND results.term ='$term' AND results.session ='$year' AND results.reg = '$reg'  AND results.`open` = 'no' AND student.reg = student_class.student and subject_id='$subject_id' and subject='$subject' order by subject desc ")or die(mysql_query());
...
{
	global $reg;
	 $rs1=mysql_query("select * from student_class where student='$reg' ") or die(mysql_error());	  
while($row1 = mysql_fetch_array($rs1)){
		$grd = $row1['level'];
		$class = $row1['class'];}
...
{
global $reg;
$rs1=mysql_query("select * from remarks where student='$reg' and session='$year' and term='$term' and subject_id='$subject_id' ") or die(mysql_error());	  
while($row1 = mysql_fetch_array($rs1)){
		$remark= $row1['remark'];
		}
return  $remark;
...
{
global $reg;
$rs1=mysql_query("select * from average where student='$reg' and session='$year' and term='$term' and subject_id='$subject_id' ") or die(mysql_error());	  
while($row1 = mysql_fetch_array($rs1)){
		$remark= $row1['average'];
		}
return  $remark;
...
function studentavg($subject_id,$year,$term){
	global $reg;
 $avgcheck = mysql_query("SELECT * FROM average where session='$year' and term='$term' and student='$reg' and subject_id = '$subject_id' ")or die(mysql_query());
 ...
 while($rwavg = mysql_fetch_array($avgcheck, MYSQL_ASSOC)){
  ...
  return $avgintable;
    ...
    function shona(){
		global $reg;
 $avgcheck = mysql_query("SELECT * FROM student_class where  student='$reg'  ")or die(mysql_query());
   $avgnum = mysql_num_rows($avgcheck);
 while($rwavg = mysql_fetch_array($avgcheck, MYSQL_ASSOC)){
