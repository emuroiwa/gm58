...
$rowpending = mysql_num_rows($pending); 
 ...
 $session = date('Y');
 for($a=1;$a<8;$a++){
 $result1 = mysql_query("SELECT * FROM class,users,final where class.teacher=users.username and class.teacher=final.ecnumber and year='$_SESSION[year]' and term='$_SESSION[term]' and class.level='$a' group by class.level order by name asc,level asc")or die(mysql_query());}
	...
	if($rows==0)
 {
 	echo("<h5><font color='#FF0000'><br><br><br><br><br>No Student Reports have finalized by respective class teachers<br><br><br><br><br> </font></h5>");  
			exit; 	  
 }?><h5>Please click on each class status to view students in each class</h5><table border="1"><tr bgcolor="#FFFFFF"><td><strong>Class</strong></td><td><strong>STATUS</strong></td></tr><?php
...
print "<tr><td>Grade <strong>$level</strong> Stream </td><td bgcolor='white'><a href='head.php?page=head_students.php&level=$level&class=$class&ec=$id&name=$name&surname=$surname 'target='_blank'>  $status</a></td></tr>
